...
if not Init(b'', ENCODING.UTF8_CODE,
            b''):  # use this line for python 3.x; for python3, string parameters should be bytes type
...
def processtag(sentence):
    ...
    sentence = sentence[1].encode("UTF8")
    tagwords = Seg(sentence)
    sqlinsert = "INSERT INTO postag (word, postag, reviewsentementid) VALUES (%s, %s, %s)"
    with con.cursor() as cursor:
        for tagword in tagwords:
            cursor.execute(sqlinsert, (tagword[0], tagword[1], sentenceid))
